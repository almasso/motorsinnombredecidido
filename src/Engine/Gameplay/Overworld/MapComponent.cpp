...
bool MapComponent::init() {
    ...
    if (_collider == nullptr) {
        Error::ShowError("Map object sin Collider", "Todos los Mapas requieren de un componente Collider para funcionar");
        return false;
    }
    Entity* ent = _scene->getEntityByHandler("Manager");
    if (ent == nullptr || !ent->getComponent<OverworldManager>()) {
        Error::ShowError("No hay OverworldManager en escena", "Todos los mapas requieren de un OverworldManager para funcionar");
        return false;
    }
    _manager = ent->getComponent<OverworldManager>();
    ent = _scene->getEntityByHandler("Player");
    if (ent == nullptr || !ent->getComponent<Collider>()) {
        Error::ShowError("No hay jugador en escena", "Todos los mapas requieren de un jugador para funcionar");
        return false;
    }
