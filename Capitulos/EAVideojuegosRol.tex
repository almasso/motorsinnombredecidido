...
\chapterquote{En mi tarjeta de presentación, soy presidente de una empresa. En mi mente, soy desarrollador de videojuegos. Pero en mi corazón, soy un jugador.}{Satoru Iwata}
...
\figura{Vectorial/botw}{width=0.7\textwidth}{fig:botw}{Escena de juego de \cite{BOTW}, extraída de \cite{BOTWForbes}.}

\cite{FailGeneros} argumentan que las categorías existentes a día de hoy se quedan cortas para satisfacer los propósitos del género en videojuegos (identidad, agrupación, \textit{marketing} y educación), ya que dada la gran diversidad de juegos actuales, resulta casi imposible poder agrupar la naturaleza multifacética de estos en etiquetas tan \comillas{tradicionales}.
...
\figura{Vectorial/dnd1975}{width=0.6\textwidth}{fig:dnd1975}{Interfaz de \cite{dnd}, extraída de \cite{dndimage}.}
...
\begin{itemize}
	...
	\item \textbf{Director creativo}: supervisa la visión general del juego y asegura que todos los elementos (jugabilidad, arte, narrativa\ldots) estén alineados con el concepto original.
\end{itemize}
...
\subsubsection{Patrones ECS y EC}
...
\begin{itemize}
	\item Entidades: son identificadores únicos que representan objetos del juego (por ejemplo, jugadores, enemigos, proyectiles\ldots). Por sí solas, las entidades no contienen ni datos ni comportamiento.
...
Ambos patrones se utilizan habitualmente para estructurar la lógica del \textit{gameplay}, ya que permiten organizar el comportamiento de los objetos de juego de forma eficiente y escalable.
...
\begin{enumerate}
	\item Procesamiento de entrada: se recogen y procesan las entradas del jugador, ya sea desde el teclado, ratón, \textit{gamepad}, red\ldots
	\item Actualización del estado del juego: se actualiza la lógica del juego, incluyendo la física, la inteligencia artificial, las colisiones y cualquier otro sistema relevante.
	\item \textit{Renderizado}: se genera la imagen correspondiente al nuevo estado del juego y se envía a la pantalla.
\end{enumerate}


Este ciclo se repite de forma constante durante toda la ejecución del juego. En juegos modernos, suele buscarse una tasa de refresco constante (por ejemplo, 60 fotogramas por segundo), aunque también pueden implementarse bucles desacoplados, donde la actualización lógica y el \textit{renderizado} ocurren a distintas frecuencias para mejorar el rendimiento o la estabilidad.
...
\begin{figure}[t]
	\begin{center}
		\begin{tikzpicture}[
			...
			\node[platform] (pc) at (-4, -1) {Motor de PC};
			\node[platform] (ps) at (0, -1) {Motor de PlayStation};
			\node[platform] (xb) at (4, -1) {Motor de Xbox};

			\node[commonlayer] (abstr) at (0, 0.5) {Interfaz común};

			\node[core] (engine) at (0, 1.75) {Componentes multiplataforma del motor};

			\node[layer] (game) at (0, 4) {Juego};

			\node[draw, thick, rounded corners, inner sep=10pt, fit=(pc) (ps) (xb) (abstr) (engine), label=left:{Motor}] (motor) {};

			\draw[arrow] (pc.north) -- ([xshift=.-4cm]abstr.south);
			\draw[arrow] (ps.north) -- (abstr.south);
			\draw[arrow] (xb.north) -- ([xshift=4cm]abstr.south);
			\draw[arrow] (game.south) -- ([xshift=-0.1cm]motor.north);
...
\figura{Vectorial/unreal}{width=0.9\textwidth}{fig:unreal}{Vista de la interfaz de \textit{Unreal Engine} 5.2, extraída de \cite{unrealinterfaz}.}
...
\figura{Vectorial/godot}{width=0.8\textwidth}{fig:godot}{Vista del \textit{scripting} visual mediante nodos de \textit{Godot}, extraída de \cite{godotnodos}.}
...
\footnotetext{En la tabla, para abreviar, PC se refiere tanto a Windows, MacOS como Linux.}
...
En la tabla \ref{tab:comparativarpg}, se encuentra una comparativa más resumida entre los distintos \textit{softwares} anteriormente mencionados con algunas de sus características más importantes.